...
def boundary_explore(train_set_X,train_set_Y,label,iteration,formu,lower_bound, upper_bound,learning_rate,training_epochs):
    while True:
        point_added_number=len(train_set_X)
        assumption_points_label=1-label
        assumption_points_X,assumption_points_Y=append_assumption_points(assumption_points_label,point_added_number,formu,lower_bound, upper_bound)
